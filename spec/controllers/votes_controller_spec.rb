...
RSpec.describe VotesController, type: :controller do
  ...
  context "signed in user" do
    ...
    describe "POST up_vote" do 
      it "the users first vote increase the number of post votes by 1" do
  ...
  describe "POST down_vote" do
    it "the user first vote increases the number of post votes by 1" do
      votes = user_post.votes.count
      post :down_vote, params: { post_id: user_post.id }
      expect(user_post.votes.count).to eq(votes + 1)
    end

    it "the user second votes does not increase the number of post votes" do
      post :down_vote, params: { post_id: user_post.id }
      votes = user_post.votes.count
      post :down_vote, params: { post_id: user_post.id }
      expect(user_post.votes.count).to eq(votes)
